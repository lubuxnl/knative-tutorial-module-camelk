...
Apart from the tools you have xref:knative-tutorial:ROOT:01-setup.adoc#prerequisite[installed], you might need few more for camelk sections of the tutorial.
...
include::partial$source-code.adoc[tag=camel-k-demos]
...
kubectl apply -n {tutorial-namespace} -f link:{github-repo}/apps/minio-s3-server/skaffold.yaml[skaffold.yaml]
----
...
----
cd $TUTORIAL_HOME/apps/minio-s3-server
oc apply -n {tutorial-namespace} -f link:{github-repo}/apps/minio-s3-server/skaffold.yaml[skaffold.yaml]
----
...
includepartial$minio-and-s3.adoc[tag=minio-aws-config]
...
[source,bash,subs="+macros,+attributes"]
----
kamel install -n {tutorial-namespace} --runtime-version='0.3.1-SNAPSHOT' # <1>
----
